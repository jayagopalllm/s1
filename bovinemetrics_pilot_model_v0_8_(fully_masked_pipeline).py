...
side_filename_to_id = {img['file_name'].lower(): img['id'] for img in coco_side.dataset['images']}
top_filename_to_id = {img['file_name'].lower(): img['id'] for img in coco_top.dataset['images']}

# --- Create a unified DataFrame with valid paths ---
data_records = []
for _, row in metadata_df.iterrows():
    ...
    # *** FIX: Explicitly check for NaN/empty strings and use placeholder path ***
...
processed_tabular = preprocessor.fit_transform(full_df[categorical_features + numerical_features].astype(str))
...
def apply_mask(image_path_str, coco_instance, filename_to_id_map):
    ...
    if image is None:
        print(f"WARNING: Could not read image at {image_path_str}. Using placeholder.")
        return np.zeros((IMG_HEIGHT, IMG_WIDTH, 3), dtype=np.uint8)

    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    filename = os.path.basename(image_path_str).lower()
...
# ==============================================================================
# STEP 5: BUILD THE MULTI-VIEW MODEL WITH AUGMENTATION LAYERS
# ==============================================================================
...
FINE_TUNE_EPOCHS = 50
total_epochs = history.epoch[-1] + FINE_TUNE_EPOCHS if history.epoch else FINE_TUNE_EPOCHS
...
# Recreate the validation dataset without shuffling for consistent evaluation
...
# Recreate the full dataset without shuffling for prediction
...
error_df['actual_weight_kg'] = error_df['weight'] # Use the original weight column
...
print(error_df_sorted[['side_view_filename', 'actual_weight_kg', 'predicted_weight_kg', 'error_kg']].head(20))
print("---------------------------------")

plt.figure(figsize=(10, 10))
plt.scatter(error_df['actual_weight_kg'], error_df['predicted_weight_kg'], alpha=0.6, edgecolors='k')
lims = [ np.min([error_df['actual_weight_kg'], error_df['predicted_weight_kg']]),
         np.max([error_df['actual_weight_kg'], error_df['predicted_weight_kg']]) ]
